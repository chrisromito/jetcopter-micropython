...
class Display(object):
    ...
    def __init__(self, spi, cs, dc, rst,
        ...
        self.write_cmd(
            self.GMCTRP1, 0x0F, 0x31, 0x2B, 0x0C, 0x0E, 0x08, 0x4E,
            0xF1, 0x37, 0x07, 0x10, 0x03, 0x0E, 0x09, 0x00
            )
        self.write_cmd(
            self.GMCTRN1, 0x00, 0x0E, 0x14, 0x03, 0x11, 0x07, 0x31,
    ...
    def draw_text(self, x, y, text, font, color, background=0,
        ...
        for letter in text:
            ...
            w, h = self.draw_letter(
                x, y, letter, font, color, background,
                landscape
                )
            # Stop on error
            if w == 0 or h == 0:
    ...
    def is_off_grid(self, xmin, ymin, xmax, ymax):
        ...
        if xmax >= self.width:
            print(
                'x-coordinate: {0} above maximum of {1}.'.format(
                    xmax, self.width - 1
                )
            )
            return True
        if ymax >= self.height:
            print(
                'y-coordinate: {0} above maximum of {1}.'.format(
                    ymax, self.height - 1
                )
            )
            return True
